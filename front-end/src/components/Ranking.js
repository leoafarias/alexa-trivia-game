...
class Ranking extends Component {
  renderRanking = () => {
    ...
    return rankedPlayers.map(function(player, index) {
      ...
      if (pos === 1) {
        trophy = <i className="gold fa fa-trophy" />;
      }
      if (pos === 2) {
        trophy = <i className="silver fa fa-trophy" />;
      }
      if (pos === 3) {
        trophy = <i className="bronze fa fa-trophy" />;
      }

      pos++;

      return (
        <li key={index} className={pos <= 4 ? "big" : ""}>
          {trophy} {player.name} <span>{player.score}</span>
        </li>
      );
    });
  };
